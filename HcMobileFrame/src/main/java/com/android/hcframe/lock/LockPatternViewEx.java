...
public class LockPatternViewEx extends View {
    ...
    public LockPatternViewEx(Context context, AttributeSet attrs) {
        ...
        mPaint.setColor(getResources().getColor(R.color.lock_bg));
...
//        final CellState cellState = mCellStates[cell.column][cell.row];
...
//        announceForAccessibility(getContext().getString(resId));
    ...
    private float getDegrees(float startX, float startY, float endX, float endY) {
		float ax = startX;// a.index % 3;
		float ay = startY;// a.index / 3;
		float bx = endX;// b.index % 3;
		float by = endY;// b.index / 3;
		float degrees = 0;
	...
	private List<Float> mDegrees = new ArrayList<Float>();
...
//                    CellState state = mCellStates[cell.column][cell.row];
                    currentPath.rewind();
                    ...
                    float d = getDegrees(lastX, lastY, centerX, centerY);
                    mDegrees.add(d);
...
//                float centerY = getCenterYForRow(j);
        ...
        if (mPatternDisplayMode == DisplayMode.Wrong) { // 画三角形
        	int length = mDegrees.size();
        	CellState cellState = mCellStates[0][0];
        	float size = cellState.size * cellState.scale;
        	mStrokePaint.setStyle(Style.FILL);
        	mStrokePaint.setColor(getCurrentColor(true));
            for (int i = 0; i < length; i++) {
    			float centerX = getCenterXForColumn(mPattern.get(i).column);
    			float centerY = getCenterYForRow(mPattern.get(i).row);
    			canvas.save();
        		canvas.rotate(mDegrees.get(i), centerX, centerY);
        		mTriangle.rewind();
        		mTriangle.moveTo(centerX + size / 2 + 30, centerY);
        		mTriangle.lineTo(centerX + size / 2 + 10, centerY - 10);
        		mTriangle.lineTo(centerX + size / 2 + 10, centerY + 10);
        		mTriangle.close();
...
//        canvas.drawCircle(centerX, centerY, size / 2, mPaint);
        ...
        mStrokePaint.setColor(getCurrentColor(partOfPattern));
        mStrokePaint.setAlpha((int) (alpha * 255));
        mStrokePaint.setStyle(Style.STROKE);
        canvas.drawCircle(centerX, centerY, size * 2, mStrokePaint);
        if (partOfPattern) {
        	mStrokePaint.setStyle(Style.FILL);
            canvas.drawCircle(centerX, centerY, size / 2, mStrokePaint);
//            HcLog.D(" drawCircle centerX = "+centerX + " centerY = "+centerY);
//            if (mPatternDisplayMode == DisplayMode.Wrong) { // 画三角形
//            	if (mIndex >= 0 && mIndex < mDegrees.size()) {
//            		centerX = getCenterXForColumn(mPattern.get(mIndex).column);
//            		centerY = getCenterYForRow(mPattern.get(mIndex).row);
//            		canvas.save();
//            		canvas.rotate(mDegrees.get(mIndex), centerX, centerY);
//            		mTriangle.rewind();
//            		mTriangle.moveTo(centerX + size / 2 + 30, centerY);
//            		mTriangle.lineTo(centerX + size / 2 + 10, centerY - 10);
//            		mTriangle.lineTo(centerX + size / 2 + 10, centerY + 10);
...
//            		canvas.drawPath(mTriangle, mStrokePaint);
//            		canvas.restore();
//            		HcLog.D(" drawCircle degree = "+mDegrees.get(mIndex) + " centerX = "+centerX + " centerY = "+centerY);
//            		mIndex ++;
//            	}
//            }
        }
