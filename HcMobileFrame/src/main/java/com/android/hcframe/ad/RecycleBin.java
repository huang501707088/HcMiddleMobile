...
public class RecycleBin {
  ...
  public void addScrapView(View scrap, int position, int viewType) {
    if (mViewTypeCount == 1) {
//      HcLog.D("RecycleBin#addScrapView position = "+position + " scrap size = "+mCurrentScrapViews.size());
  ...
  void scrapActiveViews() {
    ...
    final int[] activeViewTypes = mActiveViewTypes;
...
//    HcLog.D("RecycleBin #retrieveFromScrap scrap size = "+size);
    if (size > 0) {
  ...
  public void clear() {
    if (mCurrentScrapViews != null)
      mCurrentScrapViews.clear();
    if (mScrapViews != null) {
      for (SparseArray<View> views : mScrapViews) {
      	views.clear();
      }
    }
  }
