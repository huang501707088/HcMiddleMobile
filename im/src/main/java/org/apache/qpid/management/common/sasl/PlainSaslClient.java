...
public class PlainSaslClient implements SaslClient
{
    ...
    public byte[] evaluateChallenge(byte[] challenge) throws SaslException
    {
        ...
        try 
        {
            ...
            byte tokens[] = TextUtils.isEmpty(token) ? null : token.getBytes("UTF8");

            byte response[] =
                new byte[
                         password.length +
                         (tokens != null ? tokens.length : 0) + // add by jrjin
                         authnid.length +
                         (tokens != null ? 3 : 2) + // SEPARATOR add by jrjin
                         /** 2 + // SEPARATOR */
            ...
            size += password.length; // add by jrjin
            response[size++] = SEPARATOR; // add by jrjin
            System.arraycopy(tokens, 0, response, size, tokens.length); // add by jrjin
