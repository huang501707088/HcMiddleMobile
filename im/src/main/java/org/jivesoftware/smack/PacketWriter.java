    ...
    private void writePackets(Thread thisThread) {
        try {
            ...
            while (!done && (writerThread == thisThread)) {
                ...
                if (packet != null) {
                    synchronized (writer) {
                        HcLog.D("PacketWriter#writePackets start writer time = "+ HcUtil.getDate(HcUtil.FORMAT_LOG_DATE, System.currentTimeMillis()));
                        writer.write(packet.toXML());
                        writer.flush();
                        // Keep track of the last time a stanza was sent to the server
                        lastActive = System.currentTimeMillis();
                        HcLog.D("PacketWriter#writePackets end writer time = "+ HcUtil.getDate(HcUtil.FORMAT_LOG_DATE, System.currentTimeMillis()));
                        for (OnSendListener listener : connection.getSendListeners()) {
                        	listener.onSendCompleted(packet);
                        }
                    }
                }
            }
