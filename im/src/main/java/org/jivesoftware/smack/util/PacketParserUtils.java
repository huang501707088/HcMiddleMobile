...
//        HcLog.D("PacketParserUtils#parseMessage start start start !!!!!!!!!!!!!!!!!!!!!!!!!!!");
        ...
        while (!done) {
            ...
            if (eventType == XmlPullParser.START_TAG) {
                ...
                } else if (elementName.equals("attachment")) {
//                    String xmlLang = getLanguageAttribute(parser);
//                    if (xmlLang == null) {
//                        xmlLang = defaultLanguage;
//                    }

                    String attachment = parseContent(parser);
                    message.setAttachment(attachment);

                } else if (elementName.equals("chatType")) {
//                    String xmlLang = getLanguageAttribute(parser);
//                    if (xmlLang == null) {
//                        xmlLang = defaultLanguage;
//                    }
                    String chatType = parseContent(parser);
                    HcLog.D("PacketParserUtils #parseMessage chatType =  "+chatType);
                    message.setMediaType(chatType);
                } else if (elementName.equals("duration")) {
//                    String xmlLang = getLanguageAttribute(parser);
//                    if (xmlLang == null) {
//                        xmlLang = defaultLanguage;
//                    }
                    String duration = parseContent(parser);
...
//                        xmlLang = defaultLanguage;
//                    }
                    String userId = parseContent(parser);
...
//                        xmlLang = defaultLanguage;
//                    }
                    String receiver = parseContent(parser);
                ...
                } else if (elementName.equals("x") && namespace.equals("http://jabber.org/protocol/muc#user")) {
                    message.addExtension(new MUCUserProvider().parseExtension(parser));
                }
...
//        HcLog.D("PacketParserUtils#parseMessage end end end!!!!!!!!!!!!!!!!!!!!!!!!!!!");
        return message;
    }
        ...
        while (!done) {
            ...
            if (eventType == XmlPullParser.START_TAG) {
                        ...
                        namespace.equals(PROPERTIES_NAMESPACE))
                {
                    ...
                    presence.addExtension(parseMUCUSerPacketExtension(elementName, parser));
                }
                // Otherwise, it must be a packet extension.
                else {
    ...
    public static IQ parseIQ(XmlPullParser parser, Connection connection) throws Exception {
        ...
        while (!done) {
            ...
            if (eventType == XmlPullParser.START_TAG) {
                String elementName = parser.getName();
                String namespace = parser.getNamespace();
                HcLog.D("PacketParseUtils#parseIQ namespace ="+namespace + " elementName = "+elementName);
                ...
                } else if (elementName.equals("query") && namespace.equals("http://jabber.org/protocol/disco#items")) {
                    iqPacket = parseDiscover(parser);
                } else if (elementName.equals(/*"query"*/"x") && namespace.equals(/*"http://jabber.org/protocol/muc#owner"*/"jabber:x:data")) {
//                    String formType = parser.getAttributeValue("", "type");
                    HcLog.D("PacketParseUtils#parseIQ 房间的配置表单!");
                    iqPacket = parseDataForm(parser);
                }
                ...
                // this element name and namespace.
                    ...
                    HcLog.D("PacketParseUtils#parseIQ 命名空间不匹配 provider = "+provider);
                    if (provider != null) {
            ...
            throws Exception
    {
        ...
        HcLog.D("PacketParserUtils#parsePacketExtension elementName = "+elementName + " namespace ="+namespace);
    ...
    private static DiscoverItems parseDiscover(XmlPullParser parser) throws Exception {
        DiscoverItems discover = new DiscoverItems();
        boolean done = false;
     ...
     * @throws Exception if a parsing error occurs.
     */
    private static PacketExtension parseMUCUSerPacketExtension(String elementName, XmlPullParser parser)
            throws Exception {

        MUCUser extension = new MUCUser();
        MUCUser.Item item;
        boolean done = false;
        while (!done) {
            int eventType = parser.next();
            if (eventType == XmlPullParser.START_TAG) {
                ...
                if (name.equals("item")) {
                    ...
                    item = new MUCUser.Item(affiliation, role);
                    extension.setItem(item);
                } else if (name.equals("status")) {
                    String status = parser.getAttributeValue("", "code");
                    MUCUser.Status code = new MUCUser.Status(status);
                    extension.setStatus(code);
                }
                // Otherwise, get the the element text.
                else {
                    ;
    ...
    private static MUCOwner parseDataForm(XmlPullParser parser) throws Exception {
        DataFormProvider provider = new DataFormProvider();
        DataForm form = (DataForm) provider.parseExtension(parser);
        MUCOwner owner = new MUCOwner();
