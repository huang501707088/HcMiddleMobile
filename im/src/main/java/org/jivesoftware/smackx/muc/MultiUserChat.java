...
public class MultiUserChat {
    ...
    /**这里的静态方法快逻辑顺序有问题,被调用的时候ServiceDiscoveryManager.getInstanceFor(connection)这个为空
     * 这快东西永远调用不到
     * ServiceDiscoveryManager.init的时候,XMPPConnection又必须连接成功才能设置
    static {
    ...
    public static Collection<HostedRoom> getHostedRooms(Connection connection, String serviceName)
        ...
        ServiceDiscoveryManager discoManager = ServiceDiscoveryManager.getInstanceFor(connection);
        HcLog.D("MultiUserChat#getHostedRooms ServiceDiscoveryManager manager = "+discoManager);
    ...
    */
    ...
    private static class InvitationsMonitor implements ConnectionListener {
        ...
        private void fireInvitationListeners(String room, String inviter, String reason, String password,
            ...
            synchronized (invitationsListeners) {
                HcLog.D("MultiUserChat#fireInvitationListeners invitationsListeners size = "+invitationsListeners.size());
        ...
        private void init() {
            ...
            invitationPacketListener = new PacketListener() {
                public void processPacket(Packet packet) {
                    HcLog.D("MUltiUserChat$InvitationsMonitor#init#processPacket !!!!!!!!!!!!!!!!!!!!!!!! packet "+packet);
            ...
            HcLog.D("MultiUserChat#init!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1 listener = "+invitationPacketListener);
     ...
     * was kicked from the room.
     *
     * @param nicknames the nickname of the participant or visitor to kick from the room
     * (e.g. "john").
     * @param reason the reason why the participant or visitor is being kicked from the room.
     * @throws XMPPException if an error occurs kicking the occupant. In particular, a
     *      405 error can occur if a moderator or a user with an affiliation of "owner" or "admin"
     ...
     *      400 error can occur if the provided nickname is not present in the room.
     */
    public void kickParticipant(Collection<String> nicknames, String reason) throws XMPPException {
        changeRole(nicknames, "none");
    }
     ...
     * @throws XMPPException if an error occured while trying to discover information of a room.
     */
    public static RoomInfo getRoomInfo(Connection connection, String room, String node)
            throws XMPPException {
        DiscoverInfo info = ServiceDiscoveryManager.getInstanceFor(connection).discoverInfo(room, node);
        return new RoomInfo(info);
    }
}
