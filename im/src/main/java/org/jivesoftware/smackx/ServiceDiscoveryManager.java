...
public class ServiceDiscoveryManager {
    ...
    /**
     * 静态块是在使用到类的时候才会去加载,所以在连接的流第一次创建好的时候,这里都还没有被调用到.
     * 采用直接创建的方式进行
    static {

        HcLog.D(TAG + "#static调用静态块 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
        // Add service discovery for normal XMPP c2s connections
        XMPPConnection.addConnectionCreationListener(new ConnectionCreationListener() {
            public void connectionCreated(Connection connection) {
                HcLog.D(TAG + "#static#addConnectionCreationListener!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! connection = "+connection);
    ...
    private void init() {
        ...
        HcLog.D(TAG + " #init!!!!!!!!!!!!!!!!!!!!!!!! before instances size = "+instances.size() + " connection  = "+connection);
        instances.put(connection, this);
        HcLog.D(TAG + " #init!!!!!!!!!!!!!!!!!!!!!!!! after instances size = "+instances.size() + " connection  = "+connection);
        // Add a listener to the connection that removes the registered instance when
        // the connection is closed
        connection.addConnectionListener(new ConnectionListener() {
            public void connectionClosed() {
                // Unregister this instance since the connection has been closed
                HcLog.D(TAG + " #init#connectionClosed!!!!!!!!!!!!!!!!!!!!!!!! before remove size = "+instances.size() + " connection  = "+connection);
                instances.remove(connection);
                HcLog.D(TAG + " #init#connectionClosed!!!!!!!!!!!!!!!!!!!!!!!! after remove instances size = "+instances.size() + " connection  = "+connection);
            }
        ...
        PacketListener packetListener = new PacketListener() {
            public void processPacket(Packet packet) {
                HcLog.D(TAG + "#init#processPacket !!!!!!!!!!!!!!  packet = "+packet);
    ...
    public DiscoverItems discoverItems(String entityID, String node) throws XMPPException {
        ...
        HcLog.D(TAG + "#discoverItems result = "+result);
