package com.android.hcframe.im.voice;
...
public class Recorder implements OnCompletionListener, OnErrorListener {
    ...
    public Recorder(String fileDir) {
    ...
    public void startRecording(int outputfileformat, String extension, Context context, String fileName) {
        stop();
        
        if (/*mSampleFile == null*/true) {
            File sampleDir = new File(mFileDir);
            if (!sampleDir.canWrite()) // Workaround for broken sdcard support on the device.
                sampleDir = new File("/sdcard/sdcard");
            
            try {
                mSampleFile = File.createTempFile(fileName, extension, sampleDir);
                HcLog.D("Recorder #startRecording filePath = "+mSampleFile.getAbsolutePath());
            } catch (IOException e) {
