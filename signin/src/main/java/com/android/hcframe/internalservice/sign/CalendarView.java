...
//        drawDownOrSelectedBg(canvas);
        ...
        int todayIndex = -1;
        calendar.setTime(curDate);
        HcLog.D("CalendarView#curDate=" + curDate);
        String curYearAndMonth = calendar.get(Calendar.YEAR) + ""
                + calendar.get(Calendar.MONTH);
        calendar.setTime(today);
        HcLog.D("CalendarView#today=" + today);
    ...
    private void calculateDate() {
        HcLog.D("calculateDate#curDates=" + curDate + " " + getCurDate());
        calendar.setTime(curDate);
         ...
         * last month
         */
        HcLog.D("calculateDate#next month curStartIndex=" + curStartIndex);
        if (monthStart > 0) {
         ...
         * this month
         */
        HcLog.D("calculateDate#this month curDate=" + curDate);
        calendar.setTime(curDate);
        ...
        HcLog.D("calculateDate#next month monthDay=" + monthDay);
        curEndIndex = monthStart + monthDay;
        HcLog.D("calculateDate#this month curEndIndex=" + curEndIndex);
        /**
         * next month
         */
        HcLog.D("calculateDate#next month curEndIndex=" + curEndIndex);
        for (int i = monthStart + monthDay; i < 42; i++) {
            date[i] = i - (monthStart + monthDay) + 1;
        }
        HcLog.D("calculateDate#date[]=" + date[0] + " " + date[41]);
        ...
        HcLog.D("calculateDate#next month showLastDate=" + showLastDate);
...
//    }
    ...
    public String clickLeftMonth(Date curDates) {
        HcLog.D("CalendarView#clickLeftMonth curDate=" + curDate);
        calendar.setTime(curDates);
        calendar.add(Calendar.MONTH, -1);
        curDate = calendar.getTime();
        HcLog.D("CalendarView#calendar.getTime =" + curDate);
        invalidate();
    ...
    public String clickRightMonth(Date curDates) {
        HcLog.D("CalendarView#clickRightMonth curDate=" + curDates);
        calendar.setTime(curDates);
        calendar.add(Calendar.MONTH, 1);
        curDate = calendar.getTime();
        invalidate();
    ...
    public boolean onTouch(View v, MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                HcLog.D("CalendarView# Action_Up downDate = " + downDate + " listener = " + onItemClickListener);
                if (downDate != null) {
                    /**r
                     * 响应监听事件
                     */
    ...
    public String moveToMonth(Date curDates) {
        HcLog.D("CalendarView#moveToMonth curDate=" + curDates);
        curDate = curDates;
        invalidate();
        return getYearAndmonth();
    }
