...
public class MessagingController implements Runnable {
    ...
    void listFolders(final long accountId, MessagingListener listener) {
        ...
        if (account == null) {
            HcLog.D(EmailUtils.DEBUG, "MessagingController#listFolders Could not load account id " + accountId
                    + ". Has it been removed?");
            return;
        }
        HcLog.D(EmailUtils.DEBUG, "MessagingController#listFolders accountId ="+accountId + " MessagingListener = "+listener);
    ...
    public void synchronizeMailbox(final Account account,
         ...
         */
        HcLog.D(EmailUtils.DEBUG, "MessageingController#synchronizeMailbox folder type = "+folder.mType + " folder id = "+folder.mId);
    ...
    private void synchronizeMailboxSynchronous(final Account account,
        ...
        try {
            ...
            if (results == null) {
                 ...
                 *@date 17-4-6 下午3:08
                 */
                mListeners.synchronizeMailboxFailed(account.mId, folder.mId, new MessagingException("获取数据失败"));
                return;
            }
            ...
            if (Logging.LOGD) {
                HcLog.D(EmailUtils.DEBUG, "MessageingController#synchronizeMailbox error = "+e);
            }
    ...
    private SyncResults synchronizeMailboxGeneric(final Account account, final Mailbox mailbox)
        ...
        HcLog.D(EmailUtils.DEBUG, "MessageingController#synchronizeMailboxGeneric *** synchronizeMailboxGeneric start***");
...
//                    mListeners.sendPendingMessagesStarted(account.mId, messageId);
                    ...
                    if (Utility.hasUnloadedAttachments(mContext, messageId)) {
                        updateOutboxMessageStatus(messageId, 2);
                        mListeners.sendPendingMessagesFailed(account.mId, messageId, new MessagingException("附件未下载!"));
                        continue;
                    }
                    updateOutboxMessageStatus(messageId, 1);
                    mListeners.sendPendingMessagesStarted(account.mId, messageId);
                ...
                updateOutboxMessageStatus(messageId, 3);
                Uri syncedUri =
    ...
    private void updateOutboxMessageStatus(long messageId, int status) {
        HcLog.D(EmailUtils.DEBUG, "MessagingController#updateOutboxMessageStatus messageId = "+messageId);
        Uri uri = ContentUris.withAppendedId(EmailContent.Message.CONTENT_URI, messageId);
        ContentValues cv = new ContentValues();
        cv.put(EmailContent.MessageColumns.OUTBOX_STATUS, status);
        mContext.getContentResolver().update(uri, cv, null, null);
    }
}
