...
public class MailboxFinder {
    ...
    private class ControllerResults extends Controller.Result {
        ...
        public void updateMailboxListCallback(MessagingException result, long accountId,
            ...
            HcLog.D(EmailUtils.DEBUG, "MailboxFinder$ControllerResults#updateMailboxListCallback");
            if (result != null) {
    ...
    private class FindMailboxTask extends EmailAsyncTask<Void, Void, Long> {
        ...
        protected Long doInBackground(Void... params) {
            ...
            HcLog.D(EmailUtils.DEBUG, "MailboxFinder$FindMailboxTask#doInBackground mailboxId = "+mailboxId);
        ...
        protected void onSuccess(Long mailboxId) {
            switch (mResult) {
                case RESULT_ACCOUNT_SECURITY_HOLD:
                    HcLog.D(EmailUtils.DEBUG, "MailboxFinder$FindMailboxTask: Account security hold.");
                    try {
                ...
                case RESULT_ACCOUNT_NOT_FOUND:
                    HcLog.D(EmailUtils.DEBUG, "MailboxFinder$FindMailboxTask: Account not found.");
                    try {
                ...
                case RESULT_MAILBOX_NOT_FOUND:
                    HcLog.D(EmailUtils.DEBUG, "MailboxFinder$FindMailboxTask: Mailbox not found.");
                    try {
                        mCallback.onMailboxNotFound(mAccountId);
                    } finally {
                        close();
                ...
                case RESULT_MAILBOX_FOUND:
                    HcLog.D(EmailUtils.DEBUG, "MailboxFinder$FindMailboxTask: mailbox found: id=" + mailboxId);
                    try {
                ...
                case RESULT_START_NETWORK_LOOK_UP:
                    ...
                    HcLog.D(EmailUtils.DEBUG, "MailboxFinder$FindMailboxTask: Starting network lookup.");
