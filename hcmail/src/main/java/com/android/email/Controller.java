    ...
    public void updateMailboxList(final long accountId) {
        Utility.runAsync(new Runnable() {
            ...
            public void run() {
                ...
                HcLog.D(EmailUtils.DEBUG, "Controller #updateMailboxList service = "+service);
                if (service != null) {
                    ...
                    try {
                        ...
                        // TODO Change exception handling to be consistent with however this method
                        // is implemented for other protocols
                        HcLog.D(EmailUtils.DEBUG, "Controller updateMailboxList RemoteException" + e);
                    }
                } else {
    ...
    public void updateMailbox(final long accountId, final long mailboxId, boolean userRequest) {
        ...
        if (service != null) {
           try {
                ...
                // TODO Change exception handling to be consistent with however this method
                // is implemented for other protocols
                HcLog.D(EmailUtils.DEBUG, "Controller#updateMailbox RemoteException" + e);
            }
        } else {
            // MessagingController implementation
...
//    @VisibleForTesting
    ...
    public void sendMessage(Message message) {
        ...
        if (accountId == Account.NO_ACCOUNT) {
            ...
            HcLog.D(EmailUtils.DEBUG, "Controller #sendMessage no account found for message " + messageId);
            return;
        }
    ...
    public class LegacyListener extends MessagingListener {
        ...
        public void listFoldersStarted(long accountId) {
            HcLog.D(EmailUtils.DEBUG, TAG + "$LegacyListener #listFoldersStarted accountId = "+accountId);
            synchronized (mListeners) {
                for (Result l : mListeners) {
                    HcLog.D(EmailUtils.DEBUG, TAG + "$LegacyListener Result = "+l);
        ...
        public void listFoldersFailed(long accountId, String message) {
            HcLog.D(EmailUtils.DEBUG, TAG + "$LegacyListener #listFoldersFailed accountId = "+accountId + " message = "+message);
            synchronized (mListeners) {
                for (Result l : mListeners) {
                    HcLog.D(EmailUtils.DEBUG, TAG + "$LegacyListener Result = "+l);
        ...
        public void listFoldersFinished(long accountId) {
            HcLog.D(EmailUtils.DEBUG, TAG + "$LegacyListener #listFoldersFinished accountId = "+accountId);
            synchronized (mListeners) {
                for (Result l : mListeners) {
                    HcLog.D(EmailUtils.DEBUG, TAG + "$LegacyListener Result = "+l);
        ...
        public void synchronizeMailboxStarted(long accountId, long mailboxId) {
            HcLog.D(EmailUtils.DEBUG, TAG + "$LegacyListener #synchronizeMailboxStarted accountId = "+accountId + " mailboxId = "+mailboxId);
            synchronized (mListeners) {
                for (Result l : mListeners) {
                    HcLog.D(EmailUtils.DEBUG, TAG + "$LegacyListener Result = "+l);
        ...
        public void synchronizeMailboxFinished(long accountId, long mailboxId,
            ...
            HcLog.D(EmailUtils.DEBUG, TAG + "$LegacyListener #synchronizeMailboxFinished accountId = "+accountId + " mailboxId ="+mailboxId
                + " totalMessagesInMailbox ="+totalMessagesInMailbox + " numNewMessages = "+numNewMessages + " addedMessages="+addedMessages.toString()
                + " listeners = "+mListeners.toString());
            synchronized (mListeners) {
                for (Result l : mListeners) {
                    HcLog.D(EmailUtils.DEBUG, TAG + "$LegacyListener Result = "+l);
            ...
            HcLog.D(EmailUtils.DEBUG, TAG + "$LegacyListener#synchronizeMailboxFinished end!!!!!!!!!!!!! ");
        }

        @Override
        public void synchronizeMailboxFailed(long accountId, long mailboxId, Exception e) {
            HcLog.D(EmailUtils.DEBUG, TAG + "$LegacyListener #synchronizeMailboxFailed accountId = "+accountId + " mailboxId = "+mailboxId + " e = "+e.toString());
        ...
        public void checkMailStarted(Context context, long accountId, long tag) {
            HcLog.D(EmailUtils.DEBUG, TAG + "$LegacyListener #checkMailStarted accountId = "+accountId + " tag ="+tag);
            synchronized (mListeners) {
                for (Result l : mListeners) {
                    HcLog.D(EmailUtils.DEBUG, TAG + "$LegacyListener Result = "+l);
        ...
        public void checkMailFinished(Context context, long accountId, long folderId, long tag) {
            HcLog.D(EmailUtils.DEBUG, TAG + "$LegacyListener #checkMailFinished accountId = "+accountId + " folderId = "+folderId + " tag ="+tag);
            synchronized (mListeners) {
                for (Result l : mListeners) {
                    HcLog.D(EmailUtils.DEBUG, TAG + "$LegacyListener Result = "+l);
        ...
        public void loadMessageForViewStarted(long messageId) {
            final long accountId = Account.getAccountIdForMessageId(mProviderContext, messageId);
            HcLog.D(EmailUtils.DEBUG, TAG + "$LegacyListener #loadMessageForViewStarted accountId = "+accountId + " messageId = "+messageId);
            synchronized (mListeners) {
                for (Result listener : mListeners) {
                    HcLog.D(EmailUtils.DEBUG, TAG + "$LegacyListener Result = "+listener);
        ...
        public void loadMessageForViewFinished(long messageId) {
            final long accountId = Account.getAccountIdForMessageId(mProviderContext, messageId);
            HcLog.D(EmailUtils.DEBUG, TAG + "$LegacyListener #loadMessageForViewFinished accountId = "+accountId + " messageId = "+messageId);
            synchronized (mListeners) {
                for (Result listener : mListeners) {
                    HcLog.D(EmailUtils.DEBUG, TAG + "$LegacyListener Result = "+listener);
                    listener.loadMessageForViewCallback(null, accountId, messageId, 100);
        ...
        public void loadMessageForViewFailed(long messageId, String message) {
            final long accountId = Account.getAccountIdForMessageId(mProviderContext, messageId);
            HcLog.D(EmailUtils.DEBUG, TAG + "$LegacyListener #loadMessageForViewFailed accountId = "+accountId + " messageId = "+messageId + " message = "+message);
            synchronized (mListeners) {
                for (Result listener : mListeners) {
                    HcLog.D(EmailUtils.DEBUG, TAG + "$LegacyListener Result = "+listener);
        ...
        public void loadAttachmentStarted(long accountId, long messageId, long attachmentId,
            ...
            HcLog.D(EmailUtils.DEBUG, TAG + "$LegacyListener #loadAttachmentStarted accountId = "+accountId + " messageId = "+messageId + " attachmentId = "+attachmentId);
            try {
        ...
        public void loadAttachmentFinished(long accountId, long messageId, long attachmentId) {
            HcLog.D(EmailUtils.DEBUG, TAG + "$LegacyListener #loadAttachmentFinished accountId = "+accountId + " messageId = "+messageId + " attachmentId = "+attachmentId);
            try {
                mCallbackProxy.loadAttachmentStatus(messageId, attachmentId,
        ...
        public void loadAttachmentFailed(long accountId, long messageId, long attachmentId,
            ...
            HcLog.D(EmailUtils.DEBUG, TAG + "$LegacyListener #loadAttachmentFailed accountId = "+accountId + " messageId = "+messageId);
            try {
        ...
        synchronized public void sendPendingMessagesStarted(long accountId, long messageId) {
            HcLog.D(EmailUtils.DEBUG, TAG + "$LegacyListener #sendPendingMessagesStarted accountId = "+accountId + " messageId ="+messageId);
            synchronized (mListeners) {
                for (Result listener : mListeners) {
                    HcLog.D(EmailUtils.DEBUG, TAG + "$LegacyListener Result = "+listener);
        ...
        synchronized public void sendPendingMessagesCompleted(long accountId) {
            HcLog.D(EmailUtils.DEBUG, TAG + "$LegacyListener #sendPendingMessagesCompleted accountId = "+accountId);
            synchronized (mListeners) {
                for (Result listener : mListeners) {
                    HcLog.D(EmailUtils.DEBUG, TAG + "$LegacyListener Result = "+listener);
        ...
        synchronized public void sendPendingMessagesFailed(long accountId, long messageId,
                Exception reason) {
            MessagingException me;
            HcLog.D(EmailUtils.DEBUG, TAG + "$LegacyListener #sendPendingMessagesFailed accountId = "+accountId + " messageId="+messageId + " reason ="+reason.toString());
