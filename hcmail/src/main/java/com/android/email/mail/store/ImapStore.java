...
//    @VisibleForTesting
    ...
    private ImapFolder addMailbox(Context context, long accountId, String mailboxPath,
        ...
        HcLog.D(EmailUtils.DEBUG, "ImapStore#addMailbox mailboxPath =" +mailboxPath + " delimiter ="+delimiter + " selectable = "+selectable + " mailbox saved = "+mailbox.isSaved());
        if (mailbox.isSaved()) {
            // existing mailbox
    ...
    public Folder[] updateFolders() throws MessagingException {
        HcLog.D(EmailUtils.DEBUG, "ImapStore#updateFolders start!!!!!!!!!!!!!!!");
        ...
        try {
            ...
            HcLog.D(EmailUtils.DEBUG, "ImapStore#updateFolders responses size =" +responses.size());
            for (ImapResponse response : responses) {
                HcLog.D(EmailUtils.DEBUG, "ImapStore#updateFolders responses =" +response.toString());
                ...
                if (response.isDataResponse(0, ImapConstants.LIST)) {
                    ...
                    HcLog.D(EmailUtils.DEBUG, "ImapStore#updateFolders folderName =" +folderName);
